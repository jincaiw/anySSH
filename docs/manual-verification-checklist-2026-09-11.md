...
| 8 | 遥测关闭抓包（含对照组） | 抓包工具 + root | ☐ **本机自动化环境无法完成**（原因见下），已备好一键脚本 `scripts/telemetry-capture.sh` |
...
**当前进度（2026-09-12）**：13 项中已 ☑ **2** 项（第 4 项 macOS 部分、第 7 项），另有 **1 项部分完成**（第 6 项的 6.1 PTY 基线，见上表与 §6.1）；**第 7 项的 §7.3 兜底、第 6 项的 4 小时观测、以及第 1/2/3/5/8 项与第 4 项的 Windows/Linux 部分仍待执行**。第 9 项（原「schema 提示」）的期望值已修正，其真正内容并入第 7 项。

**下一步建议（不依赖任何外部设备、当天可完成）**：① §8 遥测关闭抓包（**必须带对照组**）；② §6.1 的 PTY 基线已可跳过（已通过），只剩挂机的 4 小时观测。其余各项所需环境目前的缺口：本机**无** Windows/Linux 虚拟机（`~/Parallels` 下无任何 `.pvm`）、5900/5901/3389 **无监听**（无 VNC/RDP 服务端）、串口仅有蓝牙与 debug-console（**无** USB 转串口）。
