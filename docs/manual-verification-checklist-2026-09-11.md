...
- 本次审计在 macOS arm64 / release 下实测：句柄 **14 → 14（+0）**、各检查点均稳定；最慢单轮关闭 **55.5 ms**、400 轮共 23.9 s（约 60 ms/轮）。
...
**当前进度（2026-09-12）**：13 项中已 ☑ **2** 项（第 4 项 macOS 部分、第 7 项），见 §7.4；**第 7 项的 §7.3 兜底、以及第 1/2/3/5/6/8 项与第 4 项的 Windows/Linux 部分仍待执行**。第 9 项（原「schema 提示」）的期望值已修正，其真正内容并入第 7 项。
