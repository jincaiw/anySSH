...
| E2E | 验证失败（修复待验证） | CI run [35854906336](https://github.com/jincaiw/anySSH/actions/runs/35854906336)：shard 2/4 为 18 passed / 1 failed，其余 3 shard 成功；失败是批量输入后控件未保留端口。后续 run [35856384656](https://github.com/jincaiw/anySSH/actions/runs/35856384656)：shard 2/4 与 3/4 各有 1 个失败，其余 2 shard 成功；新增输入一致性断言分别发现端口与用户名不完整。现已改成逐字符输入并逐字符确认，需新候选 SHA 完整 CI 复验。 |
...
- **R-13（E2E，修复待验证）**：WebKit E2E 的批量 `setValue` 在软件渲染下可能未将完整端口/用户名同步到受控输入；snippet 用例因此提交用户名 `tes`，另有 host CRUD 和 password connect 用例捕获端口/用户名不完整。`tests/e2e/helpers/host.ts` 现逐字符输入、逐字符确认，错误信息不包含 password 等字段值。需在新候选 SHA 完整 CI 验证；不降低用例断言或重试连接。
...
打候选 tag 前：取得包含逐字符 E2E 输入同步修复的新候选 SHA 全绿 CI 结果，并确认四个 E2E shard 的全部 spec 均执行完成。
