...
原候选 `f049a25` 的依赖审计发现 `RUSTSEC-2026-0285`，已更新 `Cargo.lock` 到修复版并通过本地依赖审计；升级后的候选 SHA 尚未完成完整 CI。主分支原失败分片重跑也尚未取得最终结论。Windows/Linux 干净环境安装首次启动尚无实机记录。已有验收记录显示 macOS 下载包为 ad-hoc 签名且未公证，浏览器下载的首次启动会被 Gatekeeper 拦截；GitHub 仓库当前只有 Tauri updater 签名密钥，没有 Apple Developer 签名/公证所需 secrets。以上缺口不能记录为通过。
...
| 前端构建与单测 | 已验证（代码基线） | 候选 CI run [35854211761](https://github.com/jincaiw/anySSH/actions/runs/35854211761) 的 `Frontend (typecheck + build)` job 成功；该 run 对应升级前 lockfile。升级后待重跑候选 CI。 |
| Rust fmt/clippy/test | 已验证（代码基线） | 同一 CI run 的 `Rust (fmt, clippy, test)` job 成功。 |
| 依赖安全审计 | 验证失败（修复中） | 同 SHA 的 `cargo deny check advisories licenses` 发现 `RUSTSEC-2026-0285`：`rustls 0.23.37` 存在 TLS 1.3 握手消息加密级别校验问题，可达路径包括 SSH/S3/Tauri HTTP TLS。已按 advisory 将 lockfile 更新至 `rustls 0.23.45`（修复版本）；本地复查 `advisories ok, licenses ok`。候选 SHA 仍需完整 CI 复验。 |
...
- **R-12（P1，已在候选分支修复，待 CI）**：`Cargo.lock` 的 `rustls 0.23.37` 命中 `RUSTSEC-2026-0285`。升级至 `0.23.45` 后本地 `cargo deny check advisories licenses` 通过；需候选 SHA 的完整 CI 与 Rust 回归后关闭。
...
打候选 tag 前：完成 `rustls` 安全升级并取得候选提交的必需 CI 全绿结果；取得 E2E 完整结果，确认接受或修复 E2E 不稳定问题。
