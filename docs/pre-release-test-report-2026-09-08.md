...
**残留风险（补充证据）**：并行 18 worker 的 E2E 存在约 1–3 条/74 的**随机**偶发抖动。补充数据点——同为文档提交、代码与 `2bd680e` 完全一致的 run 34176125139，Rust/Frontend 与分片 1/3/4 全绿，仅分片 2 随机失败 2 条（`26-split-pane`、`46-sftp-upload-recursive`，与前述失败 spec 均不重合），进一步证明是容器侧抖动而非代码缺陷；该作业已同样执行原样重跑。
建议后续：将 `entrypoint.sh` 的就绪判定从「TCP 端口可连」升级为「目标 sshd 实际可完成一次密码认证」，并降低单容器并行度（当前 18 worker）。
