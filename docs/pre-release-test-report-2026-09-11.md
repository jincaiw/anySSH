...
| 4. 安全检查 | **PASS**（2 WARN） | 无注入/XSS 路径；`unsafe` 0；凭据加密存储；遥测已披露，且**「关闭遥测后零出站」已抓包验证**（§12.1 V10） |
...
| 8. 可运维性 | **WARN** | `tracing` 仍无文件 appender、级别硬编码；**启动期致命错误已修**（R-9，`24e4b34`：原生对话框 + `exit(1)` + panic 落盘，实测见 §12.1 V8） |
...
- **崩溃恢复**：**无 panic hook、无 `catch_unwind`**。panic 无结构化落盘。~~**实测危害**（R-9）~~ **已修（`24e4b34`）**：启动失败不再 panic/SIGABRT，改为阻塞式原生对话框 + `exit(1)`，并新增 panic hook 追加写 `<app_data_dir>/panic.log`（`tracing` 只写 stdout ⇒ GUI 启动零日志，这条补上了「崩溃无迹可查」）。实测见 §12.1 V8。**仍未做**：`tracing` 本身依旧无文件 appender、级别仍硬编码（R-7 的其余部分）。
...
| `24e4b34` | **R-9**：启动期致命错误以 panic/SIGABRT 呈现，GUI 双击的用户看不到任何提示 | 三条启动失败路径改为 `fatal_startup_error`（打印 + 阻塞式原生对话框 + `exit(1)`）；新增 panic hook 写 `<app_data_dir>/panic.log` + 2 条单测。对话框走 `rfd` 直接调用而非插件的 `blocking_show`（后者文档明写不得在主线程调用，而 `setup()` 正是主线程）。**同时修 R-8 的文档**：README 里 `xattr -cr /Applications/anyssh.app` 路径写错（bundle 是 `anySSH.app`），且未覆盖「无法验证开发者」这一更常见的提示 |
...
| 10 | 遥测关闭抓包（**必须带对照组**，否则抓包无效不得判过） | 抓包工具 + root —— ✅ **已执行并通过**（2026-09-12，§12.1 V10） |
...
| 编号 | 验证项 | 方法 | 结果 | 证据 |
|---|---|---|---|---|
| V9 | **PTY 层长跑基线**（手册 §6.1，第 8 项的一半） | jason 在**普通终端**执行 `cargo test --release --lib -- --ignored --nocapture soak_pty_churn` | **PASS** | 400 轮：句柄 **10 → 10（+0）**；RSS 11584 → 10896 KiB（**−688 KiB**，无单调上升）；最慢单轮关闭 **61.8 ms**；总耗时 24.57 s。与本轮审计自测（句柄 14→14 +0、最慢 55.5 ms）**互证** |

**V9 的两个读数要点**（避免误判）：① 句柄**基线**两次不同（14 / 10）是正常的——要看的是**同一次运行内的增量**（均为 +0），绝对数随运行上下文变动；② 「最慢单轮关闭」是**单轮极值**而非均值，两次相差 6 ms 属噪声，都在 60 ms 量级。

**V9 同时说明一件事**：第 8 项目前只完成了「PTY 层可复现基线」，**4 小时真实混合负载观测仍未做**——工装只压本地终端层，不覆盖 SSH/SFTP/S3/端口转发并存时的应用层行为。第 8 项不能据此判为已完成。

| V10 | **遥测关闭的真实零出站**（§12 第 10 项） | `scripts/telemetry-capture.sh`：`lo0` 抓系统代理端口 + 出口接口抓端点 IP；**先跑阳性对照**，再做对照组与实验组 | **PASS** | 阳性对照（curl 走同一条路）**38** 处 → 抓包有效；对照组（默认启动）**53** 处；实验组（`ANYSSH_DISABLE_TELEMETRY=1`，同样动作）**0** 处 |

**V10 的两条环境事实**（决定了朴素 tcpdump 命令在本机必然失败）：① macOS **没有 `-i any`/`pktap`**，必须显式指定接口；② 系统代理为 `127.0.0.1:1082`，应用 HTTPS **经回环**送代理，因此目标主机名只存在于回环上那条**明文 `CONNECT`** 里，按 `host <IP>` 抓出口接口**永远抓不到**。

**V10 的方法学价值**：这一项手工做过一次，得到 0/0——`tcpdump -w` 没生成 pcap，而 `tcpdump -r <不存在文件>` 的错误被 `2>/dev/null` 吞掉，`wc -l` 返回 0。**若省掉对照组，这个假阴性会被直接读成「遥测已关闭」，结论正好相反。** 脚本因此内建阳性对照，抓不到就拒绝给结论。 |
...
- §12 的 10 项实机验证中，**第 9 项（升级 + 回滚）已完成**（§12.1 V3–V6，含应用内升级端到端与 schema 闸门），**第 6 项的 macOS 部分已完成**（§12.1 V1、V2），**第 10 项（遥测关闭抓包）已完成**（§12.1 V10），**第 8 项的 PTY 层基线已完成**（§12.1 V9）；
...
- 第 10 项已由 V10 结案，其方法（阳性对照 + 双接口抓包）已固化为 `scripts/telemetry-capture.sh`。
