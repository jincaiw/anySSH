...
**本轮改动**：6 个提交（`6377a5e` → `0fc493d`），全部已推送
**CI**：run `34609578512` @ `0fc493d` —— **8/8 job success**
...
**CI 已全绿**，但需知悉：E2E 任务存在**环境性抖动**，详见 §3.3——这是本轮发现的一项独立风险，不是代码缺陷。
...
**311 passed / 0 failed**（本轮由 308 增至 311）。同一代码在多种时序下结果完全一致：
...
| 崩溃面 | 生产 `unwrap/expect` 由 **20 处降至 13 处**（本轮修 7 处，§10）；`unsafe` 0 处；`println!/eprintln!/dbg!` 0 处 |
...
**本轮新增（泄漏面）**：`repeated_pty_lifecycle_does_not_leak_descriptors` 现在把「PTY 反复开关不泄漏描述符、teardown 必有界完成」纳入回归；桥接会话登记表的记账平衡也已被固定（§3.2）。前序已修的两处「不可取消 await」陷阱（`term/mod.rs`、`ssh/session.rs`）均有 `IO_OP_TIMEOUT` / `CHANNEL_OP_TIMEOUT` = 5s 包裹后端 I/O，关闭走 `timeout(CLOSE_TIMEOUT, &mut task)` 后 `abort()`。
...
- **长时间运行稳定性未验证**（无 >数小时连续运行记录）；内存增长趋势、线程数、连接数、句柄数的长期曲线无数据。
- **冷启动耗时未测量**——E2E 的首个用例抖动（§3.3）提示受限容器内冷启动可能超过 10s，需实机确认。
...
| R-5 | 无基准与压力测试；长跑稳定性、冷启动耗时未测量 | 中 | 未验证，见 §6 |
...
- 第 8 项（长跑）与 第 6 节的冷启动耗时，建议在正式对外前补一轮；
- 第 10 项遥测抓包须带对照组执行。

**风险知悉项**（不阻塞，但须记录在案）：E2E 环境性抖动（R-2）、前端仅能由 CI 验证（R-3）、无基准与长跑数据（R-5）、Windows GUI 零日志（R-7）。
