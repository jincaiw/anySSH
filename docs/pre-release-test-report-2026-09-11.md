...
| 编号 | 验证项 | 方法 | 结果 | 证据 |
|---|---|---|---|---|
| V8 | **R-9 修复（启动失败呈现）** | 用 `ANYSSH_DATA_DIR` 指向隔离目录 + `schema_version=21` 的库，跑 debug 二进制（**不触碰真实安装**） | **PASS** | 旧：**exit 134 / SIGABRT**、无窗口、Rust 回溯；新：**exit 1**、原生对话框**阻塞等待点击**、stderr 文案含**具体数据目录路径**。**关键方法学**：裸二进制测会**假阴性**（`CFUserNotificationDisplayAlert` 返回非零即被当取消，实测 ~4s 就返回）；放进最小 `.app` bundle 后**持续阻塞到被 kill**，据此判定发布的 `.app` 里有效 |
