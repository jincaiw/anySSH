...
enum Reply {
    AuthSuccess,
    AuthFailure {
        /// Methods the server advertised as still acceptable in the
        /// USERAUTH_FAILURE message ("authentications that can continue").
...
pub struct Handle<H: Handler> {
    ...
    /// Methods the server last advertised as acceptable (from USERAUTH_FAILURE).
    /// anySSH patch: expose the list so clients can report it to users.
    last_auth_methods: std::sync::Arc<std::sync::Mutex<Option<Vec<String>>>>,
}
...
impl<H: Handler> Handle<H> {
    ...
    /// Return (and clear) the methods the server last advertised as still
    /// acceptable in a USERAUTH_FAILURE message. `None` if no auth failure
    /// has been observed on this session.
    pub fn take_last_auth_methods(&self) -> Option<Vec<String>> {
        self.last_auth_methods
            .lock()
            .ok()
    ...
    async fn wait_recv_keyboard_interactive_reply(
        ...
        loop {
            match self.receiver.recv().await {
                ...
                Some(Reply::AuthFailure { remaining_methods }) => {
                    if let Ok(mut m) = self.last_auth_methods.lock() {
                        *m = Some(remaining_methods);
                    }
    ...
    R: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    Ok(Handle {
        ...
        join,
        last_auth_methods: std::sync::Arc::new(std::sync::Mutex::new(None)),
    })
}
