...
export function PortForwardingPage() {
  ...
  const buildContextItems = (rule: PortForwardRule): ContextMenuItem[] => {
    ...
    if (isActive) {
      items.push({
        label: t("portforward.context.stopTunnel"),
        onClick: () => void stopTunnel(rule.id),
      });
    } else {
      items.push({
        label: t("portforward.context.startTunnel"),
    ...
    <>
      <div className="flex flex-col h-full overflow-y-scroll bg-bg-base">
        <div className="max-w-4xl w-full mx-auto px-8 py-8 flex flex-col gap-8">
          ...
          {/* ── Page title ── */}
          <div>
            <h1 className="text-[length:var(--text-lg)] font-semibold text-text-primary">{t("portforward.title")}</h1>
            <p className="text-[length:var(--text-xs)] text-text-muted mt-1">{t("portforward.subtitle")}</p>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("portforward.search.placeholder")}
              aria-label={t("portforward.search.label")}
              className={[
          ...
          <div className="flex gap-2">
            <button
              ...
              title={t("portforward.newRule")}
            >
              <Plus size={14} strokeWidth={2.2} aria-hidden="true" />
              {t("portforward.newRule")}
            </button>
          </div>
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[length:var(--text-sm)] font-medium text-text-primary truncate flex-1">
                              {rule.label || t("portforward.portFallback", { port: rule.local_port })}
                            </span>

                            <button
                              role="switch"
                              aria-checked={isActive}
                              aria-label={isActive ? t("portforward.stopTunnel") : t("portforward.startTunnel")}
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              ...
                              title={t("portforward.copyAddress")}
                          ...
                          <div className="flex items-center gap-2 text-[length:var(--text-2xs)] text-text-muted/60">
                            {isActive && tunnel && tunnel.connections > 0 && (
                              <span>{t("portforward.connections", { count: tunnel.connections })}</span>
                            )}
      ...
      <ConfirmDangerDialog
        ...
        title={t("portforward.delete.title")}
        message={t("portforward.delete.message")}
...
function RuleDialog({
  ...
  return (
    <ModalShell
      open
      onClose={onCancel}
      title={isEdit ? t("portforward.dialog.editTitle") : t("portforward.dialog.newTitle")}
      icon={Plug}
      maxWidth="lg"
      scrollable
      testId="rule-dialog"
      footer={
        <>
          <button type="button" onClick={onCancel} className={BTN_GHOST}>{t("common.cancel")}</button>
          <button form="rule-dialog-form" type="submit" data-testid="rule-dialog-save" disabled={!canSubmit} className={BTN_PRIMARY}>
            {isEdit ? t("common.save") : t("portforward.dialog.create")}
          </button>
        </>
      }
    >
        <form id="rule-dialog-form" onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          <SectionHeader>{t("portforward.section.connection")}</SectionHeader>

          {/* Host */}
          <div>
            <label htmlFor="pf-host" className={labelClass}>{t("common.host")}</label>
            {hosts.length > 0 ? (
              <CustomSelect
                id="pf-host"
          ...
          <div>
            ...
            <input
              ...
              placeholder={t("portforward.placeholder.label")}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={t("portforward.placeholder.description")}
              className={`${inputClass} resize-none`}
            />
          </div>

          <SectionHeader>{t("portforward.section.ports")}</SectionHeader>
          ...
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label htmlFor="pf-local-port" className={labelClass}>{t("portforward.localPort")}</label>
              <input
                id="pf-local-port"
                ...
                placeholder={t("portforward.placeholder.port")}
                min={1}
                max={65535}
            ...
            <div className="flex-1">
              <label htmlFor="pf-remote-port" className={labelClass}>{t("portforward.remotePort")}</label>
              <input
                id="pf-remote-port"
          ...
          <SectionHeader>{t("portforward.section.options")}</SectionHeader>

          {/* Bind address */}
          <div>
            <label htmlFor="pf-bind" className={labelClass}>{t("portforward.bindAddress")}</label>
            <CustomSelect
              id="pf-bind"
              value={bindAddress}
              onChange={setBindAddress}
              options={[
                { value: "127.0.0.1", label: t("portforward.bind.localOnly") },
                { value: "0.0.0.0", label: t("portforward.bind.allInterfaces") },
              ]}
            />
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              <span className={labelClass}>{t("portforward.autoStart")}</span>
              <p className="text-[length:var(--text-2xs)] text-text-muted">{t("portforward.autoStartHint")}</p>
            </div>
            <button
              type="button"
              role="switch"
